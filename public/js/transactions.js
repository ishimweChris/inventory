...
$(document).ready(function () {
        ...
        for (let key in currentItems) {
          ...
          if (!inArray(key, selectedItemsArr)) {
            ...
            cloned
              ...
              .append(
                "<option value='" + key + "'>" + currentItems[key] + "</option>"
              );
          }
        }
  ...
  $("#modeOfPayment").change(function () {
    ...
    if (modeOfPayment === "POS" || modeOfPayment === "MOMO") {
      /**
       * Change the Label
    ...
    } else if (modeOfPayment === "Cash and MOMO") {
      /*
       * Change the label
      ...
      $("#changeDue").html("");
      $(".cashAndMomo").removeClass("hidden");
    } else if (modeOfPayment === "MOMO and POS") {
      /*
       * Change the label
       * make empty "amount tendered" field's value and also make it writable
       * unset any value that might be in "changeDue"
       * display "MOMO" an "POS" fields
       */
      $("#amountTenderedLabel").html("Total");
      $("#amountTendered").val("").prop("disabled", true);
      $("#changeDue").html("");
      $(".MomoAndPos").removeClass("hidden");
    ...
    } else if (modeOfPayment === "CASH MOMO and POS") {
      /*
       * Change the label
       * make empty "amount tendered" field's value and also make it writable
       * unset any value that might be in "changeDue"
      ...
      $("#amountTendered").val("").prop("disabled", true);
      $("#changeDue").html("");
      $(".CashAndMomoAndPos").removeClass("hidden");
    } else {
      //if cash. If something not recognise, we assume it is cash
      /*
       * change the label
  ...
  $("#cashAmount,  #posAmount").on("change", function (e) {
    ...
    var totalAmountTendered =
      parseFloat($("#posAmount").val()) + parseFloat($("#cashAmount").val());

    //set amount tendered as the value of "totalAmountTendered" and then trigger the change event on it
    $("#amountTendered")
      .val(isNaN(totalAmountTendered) ? "" : totalAmountTendered)
      .change();
  });
  ...
  /**
   * If mode of payment is "Cash and MOMO", both #cashAmount and #posAmount fields will be visible to user to add values
   * The addition of both will be set as the amount tendered
   */

  $("#MomoAmount,#cashAmount2").on("change", function (e) {
    e.preventDefault();

    var totalAmountTendered =
      parseFloat($("#MomoAmount").val()) + parseFloat($("#cashAmount2").val());
  ...
  /**
   * If mode of payment is "MOMO and POS", both #cashAmount and #posAmount fields will be visible to user to add values
   * The addition of both will be set as the amount tendered
   */

  $("#MomoAmount2,#posAmount2").on("change", function (e) {
    e.preventDefault();

    var totalAmountTendered =
      parseFloat($("#MomoAmount2").val()) + parseFloat($("#posAmount2").val());
  ...
  $("#cashAmount3,#MomoAmount3,#posAmount3").on("change", function (e) {
    e.preventDefault();

    var totalAmountTendered =
      parseFloat($("#cashAmount3").val()) +
      parseFloat($("#MomoAmount3").val()) +
      parseFloat($("#posAmount3").val());
  ...
  $("#discountValue").change(function () {
    ...
    var discountPercentage =
      (discountValue / cumTotalWithoutVATAndDiscount) * 100;
  ...
  $("#confirmSaleOrder").click(function () {
    ...
    if (
      ...
      isNaN(amountTendered) || amountTendered === "0.00"
        ? $("#amountTenderedErr").html("required")
        : $("#amountTenderedErr").html("");
      !modeOfPayment
        ? $("#modeOfPaymentErr").html("Select mode of payment")
        : $("#modeOfPaymentErr").html("");
      amountTendered < cumAmount
        ? $("#amountTenderedErr").html(
            "Amount cannot be less than " + cumAmount
          )
        : "";
      ...
      for (var i = 0; i < selectedItemNodeLength; i++) {
        ...
        if (qty === 0 || availQty < qty || expectedTotPrice !== totalPrice) {
          totalPriceNode.style.backgroundColor =
            expectedTotPrice !== totalPrice ? "red" : "";
          qtyNode.style.backgroundColor =
            qty === 0 || availQty < qty ? "red" : "";
          ...
          verifyCumAmount =
            parseFloat(verifyCumAmount) + parseFloat(totalPrice);
        }
      }
  ...
  $("#clickToGenitem").click(function (e) {
    ...
    var fromDate = $("#transFromItem").val();
    var toDate = $("#transToItem").val();
...
function gti_(transId) {
  if (transId) {
    $("#transReceipt").html(
      "<i class='fa fa-spinner faa-spin animated'></i> Loading..."
    );
...
function ceipacp() {
  ...
  $(".transItemList").each(function () {
    ...
    if (transQty > availQty) {
      ...
      $(this)
        .find(".itemTransQtyErr")
        .html("only " + availQty + " left");
      ...
      var itemTotalPrice = parseFloat(
        $(this).find(".itemUnitPrice").html() * transQty
      );
...
function calchadue() {
  ...
  if (amountTendered && amountTendered < cumAmount) {
    $("#amountTenderedErr").html("Amount cannot be less than Rwf " + cumAmount);
