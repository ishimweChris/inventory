...
$(document).ready(function () {
  ...
  $("#printDamage").click(function (e) {
    e.preventDefault();

    var strWindowFeatures =
      "width=1000,height=500,scrollbars=yes,resizable=yes";

    window.open(appRoot + "dashboard/getDamages/", "Print", strWindowFeatures);
  });
  ...
  $("#earningAndExpenseYear").change(function () {
    ...
    if (year) {
      $("#yearAccountLoading").html(
        "<i class='" + spinnerClass + "'></i> Loading..."
      );
...
function getEarnings(year) {
  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      document.getElementById("earningsTitle").innerHTML =
        "Earnings (" + response.earningsYear + ")";

      var earningsGraph = document
        .getElementById("earningsGraph")
        .getContext("2d");
