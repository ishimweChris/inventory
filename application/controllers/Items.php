...
class Items extends CI_Controller{
    ...
    */
    ...
    public function add(){
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $itemPrice = "Rwf ".number_format(set_value('itemPrice'), 2);
    ...
    */
    ...
    public function edit(){
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $itemName = set_value('itemName');
            $code = set_value('itemCode');
            $itemCode = $this->input->post('itemCode', TRUE);
            
            //update item in db
            $data = ['code'=>$code,'name'=>$itemName,'description'=>$itemDesc,'unitPrice'=>$itemPrice];
            $updated = $this->item->edit($itemId,$data);
    ...
    */
    ...
    public function delete(){
        ...
        $image = $this->item->getImageName($item_id);

        if($item_id){
            $this->db->where('id', $item_id)->delete('items');    
            $file= './assets/images/'.$image;
            unlink($file);
            $json['status'] = 1;
        }
    ...
    public function itemreport($from_date, $to_date=''){        
        ...
        $data['allItems'] = $this->item->getDateRangeItems($from_date, $to_date);
    ...
    public function edit_image()
    {
         ...
         // body of else clause will be executed when image uploading is succeeded
             ...
             $itemId = $this->input->post('itemIdEdit');
             $image = $_FILES['userfile']['name'];  //name must be userfile    
         }
         $this->item->update_image($itemId,$image);
         redirect('items');
