...
class Transactions extends CI_Controller{
    ...
    */
    ...
    public function nso_(){
        ...
        if($allIsWell){//insert each sales order into db, generate receipt and return info to client
            ...
            $eventDesc = count($arrOfItemsDetails). " items totalling Rwf ". number_format($cumAmount, 2)
    ...
    */
    ...
    public function itemreport($from_date, $to_date=''){        
        //get all transactions from db ranging from $from_date to $to_date
        $data['itemfrom'] = $from_date;
        $data['itemto'] = $to_date ? $to_date : date('Y-m-d');
        
        $data['allItems'] = $this->transaction->getDateRangeItems($from_date, $to_date);
        
        $this->load->view('transactions/itemReport', $data);
    }

       // ********************************************* 
       public function getItems(){
        $data['itemsAll'] = $this->item->getAllItems();
        $this->load->view('items/stockReport', $data);
    }

      // ********************************************* 
      public function getStore(){
        $data['transValue'] = $this->transaction->totalTransValue();
        $data['totalVat'] = $this->transaction->totalVat();
        $data['totalDiscount'] = $this->transaction->totalDiscount();
        $data['totalEarned'] = $this->transaction->totalEarned();
        $data['stockValue'] = $this->transaction->totalStockValue();
        $this->load->view('items/storeReport', $data);
    }
}
